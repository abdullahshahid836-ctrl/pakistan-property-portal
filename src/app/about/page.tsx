      ...
      <div className="overflow-x-hidden">
        ...
        <section ref={sectionRef} className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden bg-[#004737] px-4 py-10 sm:py-20">
          ...
          <div className="relative z-10 w-full max-w-[1600px] mx-auto flex flex-col items-center justify-center px-4 sm:px-10">
            {/* PUZZLE PIECE CARDS — INTERLOCKING LAYOUT */}
            <div className="w-full flex flex-col lg:flex-row items-center justify-center relative -mt-5 lg:-mt-56 gap-16 lg:gap-0">
              
              {/* Card 1: MISSION — INTERLOCKING TOP-RIGHT */}
              <motion.div
                initial={{ opacity: 0, x: -50 }}
                whileInView={{ opacity: 1, x: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 1, ease: [0.22, 1, 0.36, 1] as any }}
                className="relative w-full lg:w-[55%] max-w-[850px] z-20 group"
              >
                {/* Puzzle piece tab at top-right */}
                <div className="absolute top-0 right-0 -translate-y-full flex flex-col items-end">
                  <div className="bg-[#004737] px-10 py-4 rounded-t-3xl relative">
                    <span className="font-syne font-black text-[#C8F55A] text-[clamp(11px,1.4vw,14px)] uppercase tracking-[0.3em]">Mission</span>
                    {/* Left Inverted Corner */}
                    <div className="absolute bottom-0 -left-8 w-8 h-8 bg-transparent rounded-br-3xl shadow-[15px_15px_0_0_#004737]" />
                  </div>
                </div>

                {/* Main Card Body */}
                <div className="bg-[#004737] p-10 sm:p-20 rounded-3xl rounded-tr-none shadow-[0_40px_100px_rgba(0,0,0,0.4)] relative border border-white/5 backdrop-blur-sm">
                  <h3 className="font-syne font-black text-white uppercase text-[clamp(24px,5vw,72px)] leading-[0.9] mb-8 tracking-tighter">Our Mission<br/>Defined.</h3>
                  <p className="font-inter text-white/50 text-lg sm:text-xl leading-relaxed max-w-[500px]">Founded in 2012, Pakistan Property Portal has evolved into a comprehensive ecosystem for real estate excellence.</p>
                  
                  {/* Bottom puzzle notch (interlocks with next card) */}
                  <div className="absolute -bottom-8 right-24 w-16 h-8 bg-[#004737] rounded-b-2xl hidden lg:block" />
                </div>
              </motion.div>

              {/* Card 2: VISION — INTERLOCKING TOP-LEFT */}
              <motion.div
                initial={{ opacity: 0, y: 100 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 1, delay: 0.2, ease: [0.22, 1, 0.36, 1] as any }}
                className="relative w-full lg:w-[50%] max-w-[750px] lg:mt-[50%] lg:-ml-[5%] z-10 group"
              >
                {/* Puzzle piece tab at top-left */}
                <div className="absolute top-0 left-0 -translate-y-full flex flex-col items-start">
                  <div className="bg-[#56F09F] px-10 py-4 rounded-t-3xl relative">
                    <span className="font-syne font-black text-[#01382C] text-[clamp(11px,1.4vw,14px)] uppercase tracking-[0.3em]">Vision</span>
                    {/* Right Inverted Corner */}
                    <div className="absolute bottom-0 -right-8 w-8 h-8 bg-transparent rounded-bl-3xl shadow-[-15px_15px_0_0_#56F09F]" />
                  </div>
                </div>

                {/* Main Card Body */}
                <div className="bg-[#56F09F] p-8 sm:p-14 rounded-3xl rounded-tl-none shadow-[0_40px_100px_rgba(0,0,0,0.3)]">
                  <h3 className="font-syne font-black text-[#01382C] uppercase text-[clamp(20px,4vw,60px)] leading-[0.9] mb-8 tracking-tighter">12+ Years<br/>Experience.</h3>
                  <p className="font-inter text-[#01382C]/70 text-base sm:text-lg leading-relaxed max-w-[400px]">50,000+ active members trust Pakistan Property Portal for their biggest life milestones.</p>
                </div>
              </motion.div>
            </div>
          </div>
